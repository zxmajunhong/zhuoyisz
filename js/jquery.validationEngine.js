...
jQuery.fn.validationEngine = function(settings) {
	if($.validationEngineLanguage){					// IS THERE A LANGUAGE LOCALISATION ?
		...
		allRules = {"required":{    			  // Add your regex rules here, you can take telephone as an example
						...
						"date":{
                             ...
                             "alertText":"* 输入的信息, 必须以 YYYY-MM-DD 格式"},
						"onlyNumber":{
							"regex":"/^[0-9\ ]+$/",
							"alertText":"* 只能输入数字"},	
						"noSpecialCaracters":{
	...
	var validateCall = function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FILED
		...
		function _MaxCheckbox(caller,rules,position){    // VALIDATE CHECKBOX NUMBER 验证CheckBox的数
	...
	var closePrompt = function(caller) {	// CLOSE PROMPT WHEN ERROR CORRECTED 关闭时提示错误更正
	...
	var submitValidation = function(caller) {	// FORM SUBMIT VALIDATION LOOPING INLINE VALIDATIONFORM 提交的确认环不内插的确认
		var stopForm = false
		...
		if(stopForm){							// GET IF THERE IS AN ERROR OR NOT FROM THIS VALIDATION FUNCTIONS 如获得有错误或从本不是函数的确认
